...
// Memory Variable: -1 = Left, 1 = Right, 0 = None
int lastDirection = 0;
...
void loop() {
  ...
  // -------------------------------------------------------------
  // Decision Matrix with Last Direction Memory
  // -------------------------------------------------------------
  if (onBlack[2] || onBlack[3]) {
    ...
    turnLeft();
    lastDirection = -1; // Remember line was on the left
  }
  else if (onBlack[1]) {
    ...
    turnRight();
    lastDirection = 1;  // Remember line was on the right
  }
  else if (onBlack[4]) {
    ...
    // Line lost: spin in the last known direction for 90-degree recovery
    if (lastDirection == -1) {
      Serial.println("LINE LOST - RECOVERING LEFT");
      turnLeft();
    } 
    else if (lastDirection == 1) {
      Serial.println("LINE LOST - RECOVERING RIGHT");
      turnRight();
    } 
    else {
