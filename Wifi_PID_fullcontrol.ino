...
int masterSpeed = 140;          // Optimal base speed
float Kp = 0.08;                // Proportional constant
float Kd = 0.80;                // Derivative constant
...
<body>
  <div class="card">
    ...
    <div class="control-group">
      <label>Kp (Proportional) <span class="val" id="kpVal">0.08</span></label>
      <input type="range" min="0.00" max="0.30" step="0.01" value="0.08" oninput="updateParams()" id="kpSlider">
    </div>

    <div class="control-group">
      <label>Kd (Derivative) <span class="val" id="kdVal">0.80</span></label>
      <input type="range" min="0.00" max="3.00" step="0.05" value="0.80" oninput="updateParams()" id="kdSlider">
    </div>
  </div>
...
void setup() {
  ...
  isCalibrated = true;
  stopMotors();
  Serial.println("Calibration Complete! Waiting for START command from web UI...");
}
...
void runPID() {
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    // Normalize reading (0 = White, 1000 = Black)
  ...
  if (totalSum > 300) {
    float position = (float)weightedSum / totalSum; // Position range: 0 to 5000 (Center = 2500)
    error = position - 2500.0;
  } else {
    // Line lost: maintain hard spin in direction of last known error
    if (lastError < 0) error = -2500.0;
    else error = 2500.0;
  }
  ...
  float baseSpeedA = masterSpeed * FACTOR_A;
  float baseSpeedB = masterSpeed * FACTOR_B;
