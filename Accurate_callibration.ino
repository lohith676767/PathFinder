...
void setup() {
  ...
  // -------------------------------------------------------------
  // Countdown to allow setup (5 Seconds)
  // -------------------------------------------------------------
  ...
  // SWEEP SENSORS MANUALLY ACROSS BOTH BLACK LINE AND WHITE SURFACE NOW!
  // -------------------------------------------------------------
...
void loop() {
  ...
  for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    // Check that calibration captured a valid difference (> 400 raw ADC units)
    if (range > 400) {
      int threshold = sensorMin[i] + (range / 2);
      onBlack[i] = (sensorValues[i] > threshold);
    } else {
      // Default to white if sensor wasn't properly calibrated across a line
      onBlack[i] = false; 
    }
  ...
  // Decision Matrix
  // -------------------------------------------------------------
  if (onBlack[3] || onBlack[4]) {
    Serial.println("STRAIGHT");
    moveForward();
  }
  else if (onBlack[0] || onBlack[1]) {
